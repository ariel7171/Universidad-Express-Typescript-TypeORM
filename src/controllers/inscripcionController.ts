...
export const guardarInscripcion = async (req: Request, res: Response): Promise<void> => {
  try {
    const { curso_id, estudiante_id, nota } = req.body as Inscripcion;

    const curso: Curso | null = await cursoRepository.findOneBy({ id: Number(curso_id) });
    if (!curso) {
      res.status(404).json({ message: 'Curso no encontrado' });
      return;
    }

    const estudiante: Estudiante | null = await estudianteRepository.findOneBy({ id: Number(estudiante_id) });
    if (!estudiante) {
      res.status(404).json({ message: 'Estudiante no encontrado' });
      return;
    }

    const instanciaInscripcion: Inscripcion = inscripcionRepository.create({ curso, estudiante, nota });

    const inscripcion: Inscripcion = await inscripcionRepository.save(instanciaInscripcion);
...
export const actualizarInscripcion = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const result: UpdateResult = await queryRunner.manager.update(Inscripcion, { curso_id, estudiante_id }, { nota });
...
export const eliminarInscripcion = async (req: Request, res: Response) => {
  const { curso_id, estudiante_id } = req.body as Inscripcion;
